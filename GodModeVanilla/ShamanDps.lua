...
local function ShamanDps()
	if(CastingInfo == nil) then
		...
		if(UnitCanAttack("player", "target") and (UnitIsDeadOrGhost("target") == nil)) then
			...
			if(IsSpellReady("Horion de terre")) then
				--Earth Shock
				UseAction(GetSlot("Horion de terre"))
			elseif(IsSpellReady("Horion de givre") and not FrostShockDebuff and UnitPlayerControlled("target")) then
				--Frost Shock
				UseAction(GetSlot("Horion de givre"))
			elseif(IsSpellReady("Chaîne d'éclairs") and (AATimer > 2.0)) then
				--Chain Lightning
...
function ShamanHeal_Dps()
	...
	elseif(CastingInfo == nil and not UnitIsDeadOrGhost("player")) then
		...
		elseif(not IsPlayerSpell("Arme Furie-des-vents") and IsSpellReady("Arme Langue de feu") and not GetWeaponEnchantInfo()) then
			--Flametongue Weapon
			UseAction(GetSlot("Arme Langue de feu"))
		elseif(IsSpellReady("Arme Furie-des-vents") and not GetWeaponEnchantInfo()) then
			--Windfury Weapon
			UseAction(GetSlot("Arme Furie-des-vents"))
		...
		elseif(not IsPlayerSpell("Totem Fontaine de mana") and IsSpellReady("Totem guérisseur") and not HealingTotemBuff and Combat) then
		...
		elseif((HealTarget > 0) and (IsInRaid() == false)) then
			if(IsSpellReady("Salve de guérison") and (AATimer > 2.0) and (nbrChainHeal >= 3)) then
				--Chain Heal
...
function Shaman_Dps_OnUpdate(elapsed)
	DrinkingBuff = GetUnitBuff("player", DrinkingTexture)
	if(((PrctMana > 33) or (HasDrink() == 0)) and ((not DrinkingBuff) or (PrctMana > 80))) then FollowMultibox(TankName) end
